...
namespace Denics.FrontPage
{
    ...
    {
        private readonly CallDatabase db = new CallDatabase();
        ...
        {
            InitializeComponent();
            con = new SqlConnection(db.getDatabaseStringName());
        ...
        // First, check user email to register information by giving otp to be book
        // Second, once verified, get booking information from AppointmentCart
        // Third, send pdf for pending for clarification
        private void Book_btn_Click(object sender, EventArgs e)
        {
            ...
            // Validate by IDs (store numeric ids in AppointmentCart)
            ...
            {
                MessageBox.Show("Incomplete booking details. Please ensure all selections are made.", "Booking Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            // Proccess to check if otp is successful
            EmailVerification();
        }
        // Get booking details from ApppointmentCart
        // Add appointment to Appointments table with status 'pending'
        ...
        private void LoadAvailableServices()
        {
            ...
            const string sql = @"
                ...
                FROM Services
                WHERE status IS NOT NULL AND LOWER(status) IN ('available')
        ...
        private void RenderTimeAvailability(int doctorId, DateTime date)
        {
            ...
            try
            {
                ...
                using (var cmd = new SqlCommand(availSql, con))
                {
                ...
                using (var cmd = new SqlCommand(apptSql, con))
                {
                    cmd.Parameters.AddWithValue("@doctorId", doctorId);
                    cmd.Parameters.AddWithValue("@date", date);
                    using var r = cmd.ExecuteReader();
        ...
        {
            // get selected service name and id
            string serviceName = (ServiceType_cmbbx.SelectedItem as ComboBoxItem)?.Text ?? string.Empty;
            int selectedServiceId = (ServiceType_cmbbx.SelectedItem as ComboBoxItem)?.Id ?? -1;
            ...
            if (string.IsNullOrWhiteSpace(serviceName) || selectedServiceId <= 0)
            {
                MessageBox.Show("Please select a service first.", "Service Required", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            // store both id and user-friendly name to AppointmentCart
            AppointmentCart.SetServiceId(selectedServiceId);
            AppointmentCart.SetServiceType(serviceName);
            AppointmentCart.SetDoctorId(doctorId);
